...
#         weight.loc[self.market] += (1-self.alphaw)
    ...
    def _get_vol_exante_semi(self, date, weight, n_period, opts):
        ...
        if opts=='up':
            mm = self.r.loc[self.r.index<date, w_p.index].iloc[-n_period:].clip(lower=0) #_lower(0)
        elif opts=='down':
            mm = self.r.loc[self.r.index<date, w_p.index].iloc[-n_period:].clip(upper=0) #_upper(0)
