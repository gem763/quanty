...
class DualMomentumSelector(object):
    ...
    def _trend(self, dates):
        has_trend = has_trend_sp = has_trend_cash = has_trend_market = None
        ...
        #if (self.follow_trend_cash is not None) & (self.cash_equiv is not None):    
        #    has_trend_cash = self._has_trend(dates, self.follow_trend_cash, asset=self.cash_equiv)
    ...
    def _sig_dynamic_mix_by_n_fwd(self, dates, pr, n_backs, n_fwd):
        ...
        p2 = pr_.loc[dates[0] - pd.DateOffset(days=self.sig_dyn_n_sample):]
        perf_fut_rt = p2.pct_change(n_fwd)
        #perf_fut_std = p2.pct_change().rolling(n_fwd).std()
        #perf_fut = perf_fut_rt/perf_fut_std
        
        def _get_cor(n_back):
            perf_past_rt = p1.pct_change(n_back).loc[dates[0] - pd.DateOffset(days=self.sig_dyn_n_sample):]
            ...
            cor = perf_past_rt.corrwith(perf_fut_rt, axis=1).rolling(self.sig_dyn_n_sample, min_periods=int(self.sig_dyn_n_sample/2)).mean()
            return cor
        ...
        def __selection(date):
            ...
            if self.strong_condition:
                ...
                if sp_has_trend or sp_has_positive_sig:
                    ...
                    #cash_has_trend = self.has_trend_cash.loc[date] if self.has_trend_cash is not None else False
                    cash_has_positive_sig = self.sig.loc[date, self.cash_equiv]>=0
                    pos = self._get_default_selection(date, n_picks)

                    if cash_has_positive_sig:# or cash_has_trend:
